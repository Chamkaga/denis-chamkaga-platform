  ...
  async getSupportersOverview() {
    ...
    for (const t of tiers) {
      ...
      tierBreakdown[t] = {
        ...
        monthlyRevenue: Number(tierMonthly._sum.amount || 0),
        lifetimeRevenue: Number(tierLifetime._sum.amount || 0),
        avgContribution: Math.round(Number(tierLifetime._avg.amount || 0)),
    ...
    return {
      summary: {
        ...
        recurringSupporters,
        oneTimeSupporters,
        monthlyRevenue: Number(monthlyAggregate._sum.amount || 0),
        lifetimeRevenue: Number(lifetimeAggregate._sum.amount || 0),
        avgContribution: Math.round(Number(lifetimeAggregate._avg.amount || 0)),
        largestContribution: Number(lifetimeAggregate._max.amount || 0),
        pendingPayments,
        failedPayments,
      },
      tierBreakdown,
